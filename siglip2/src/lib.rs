...
pub const TOEKN_DIM: usize = 64;

pub const IMAGE_CHANNEL: usize = 3;
pub const IMAGE_WIDTH: usize = 384;
pub const IMAGE_HEIGHT: usize = 384;

pub const MEAN: [f64; 3] = [0.5, 0.5, 0.5];
pub const STD: [f64; 3] = [0.5, 0.5, 0.5];
...
where
{
    let encodings = tokenizer
        .encode_batch(batch_input.into_iter().collect(), false)
        .map_err(|e| anyhow!("{e}"))?;
    ...
    let output = session.run(inputs![
        "input_ids" => Tensor::from_array(([batch_size, TOEKN_DIM], input_ids))?,
    ])?;
